...
namespace nanoframework.Devices.SPI
{
    ...
    public class MAX31865
    {
        ...
        public double ReferenceReistor { get; private set; }
        ...
        public void Initialize(string spiBus, int irqPinNumber, int csPinNumber, byte config, double referenceResistor = 400, SensorType sensor = SensorType.PT100, int spiClockRateHz = 1000000)
        {
            ...
            _irqPin = GpioController.GetDefault().OpenPin(irqPinNumber); //GTI.InterruptInputFactory.Create(_socket, irqPin, GTI.GlitchFilterMode.On, GTI.ResistorMode.PullUp, GTI.InterruptMode.FallingEdge, this);
        ...
        {
            FaultScanner = new Timer(FaultScanner_callback,null,0,interval);
        }
        ...
        {
            const double x = 32768; //System.Math.Pow(2, 15); = 32768 so we will use the constant
        ...
        {
            const double RTD_ALPHA = 3.90802e-3; //ITS90 = 3.9080e-3
            const double RTD_BETA = -5.802e-7; //ITS90 = -5.870
            const double a2 = 2.0 * RTD_BETA;
            const double bSq = RTD_ALPHA * RTD_ALPHA;

            double c = 1.0 - GetResistance() / (int)Sensor; //100 for PT100, 1000 for PT1000
            double d = bSq - 2.0 * a2 * c;
