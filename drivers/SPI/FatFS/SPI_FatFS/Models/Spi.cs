...
namespace SPI_FatFS
{
    static class Spi
    {
        ...
        public static void InitSpi(string busId, int chipSelectLine)
        {
            if (device == null)
            {
                var settings = new SpiConnectionSettings(chipSelectLine)   // The slave's select pin. Not used. CS is controlled by by GPIO pin
                {
                    Mode = SpiMode.Mode0,
